...
const Manager = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get("https://messwafflespos.onrender.com/api/manager/inventory")
            .then((response) => {
    ...
    const usageSubmit = (event) => {
        ...
        axios
            .get("https://messwafflespos.onrender.com/api/manager/orders", {
                params: { date1: eventDate1, date2: eventDate2 },
            })
            .then((response) => {
    ...
    const processTrends = () => {
        ...
        const pushToMap = (a, b) => {
            ...
            if (trendsMap.has(JSON.stringify({ item1: a, item2: b }))) {
                trendsMap.set(
                    JSON.stringify({ item1: a, item2: b }),
                    trendsMap.get(JSON.stringify({ item1: a, item2: b })) + 1
                );
            } else {
    ...
    function Inventory() {
        ...
        useEffect(() => {
            axios
                .get(
                    "https://messwafflespos.onrender.com/api/manager/inventory"
                )
                .then((response) => {
        ...
        for (let i = 0; i < ings.rowCount; i++) {
            ingsList.push(
                JSON.stringify(ings.rows[i].item).substring(
                    1,
                    JSON.stringify(ings.rows[i].item).length - 1
                )
            );
        ...
        return (
            <>
                <div className="checkout">
                    <div id="itemCol" className="itemsCol">
                        Item
                    ...
                    <div id="quantCol" className="itemsCol">
                        Quantity
                        <ul style={{ display: "block" }}>
    ...
    function updateItems(e) {
        ...
        axios
            .post(
                "https://messwafflespos.onrender.com/api/manager/items",
                postdata
            )
            .then((response) => {
                console.log(response.data);
    ...
    function deleteItem(e) {
        ...
        console.log(postdata);
        axios
            .delete(`https://messwafflespos.onrender.com/api/manager/items/`, {
                data: postdata,
            })
            .then((response) => {
                console.log(response.data);
    ...
    function Items() {
        ...
        for (let i = 0; i < itemsMenu.rowCount; i++) {
            itemList.push(
                JSON.stringify(itemsMenu.rows[i].item).substring(
                    1,
                    JSON.stringify(itemsMenu.rows[i].item).length - 1
                )
            );
            priceList.push(itemsMenu.rows[i].price);
            ingList.push(itemsMenu.rows[i].ingredients);
            catList.push(
                JSON.stringify(itemsMenu.rows[i].category).substring(
                    1,
                    JSON.stringify(itemsMenu.rows[i].category).length - 1
                )
            );
        }
        ...
        return (
            <>
                <div className="checkout">
                    ...
                    <div id="ingCol" className="itemsCol">
                        Ingredients
                        <ul style={{ display: "block" }}>
                    ...
                    <div id="catCol" className="itemsCol">
                        Category
                        <ul style={{ display: "block" }}>
    ...
    function restockReport(e) {
        ...
        axios
            .get(
                "https://messwafflespos.onrender.com/api/manager/restockReport",
                { params: { quantity: q } }
            )
            .then((response) => {
                console.log(response.data);
                ...
                document.getElementById("restockReturn").innerHTML =
                    restockList;
        ...
        <main>
            <Tabs defaultActiveKey="inventory">
                ...
                <Tab eventKey="restockReport" title="Restock Report">
                    <center>
                        Enter a number to see all inventory items that need
                        restock for that threshold:

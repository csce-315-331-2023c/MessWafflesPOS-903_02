...
router.post("/items", async (req, res) => {
    const { item, price, ingredients, category, picture, description } =
        req.body;
    try {
        ...
        for (let j = 0; j < iList.length; j++) {
            ...
            const thisItem = await db.query(
                "SELECT * FROM inventory WHERE item = $1",
                [ing]
            );
            if (thisItem.rowCount == 0) {
                await db.query(
                    "INSERT INTO inventory (item, quantity) VALUES ($1, 0)",
                    [ing]
                );
            }
        }
        await db.query(
            "INSERT INTO items (item, price, ingredients, category, picture, description) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (item) DO UPDATE SET price = $2, ingredients = $3, category = $4, picture = $5, description = $6",
            [item, price, ingredients, category, picture, description]
        );
...
router.post("/setItemWeatherType", async (req, res) => {
    ...
    try {
        await db.query("UPDATE items SET weather_type = $2 WHERE item = $1", [
            item,
            weather_type,
        ]);
        res.status(201).send(`Added item ${item}`);
...
router.post("/inventory", async (req, res) => {
    ...
    try {
        await db.query(
            "INSERT INTO inventory (item, quantity) VALUES ($1, $2) ON CONFLICT (item) DO UPDATE SET quantity = $2",
            [item, quantity]
        );
...
router.get("/orders", async (req, res) => {
    ...
    try {
        const result = await db.query(
            "SELECT item FROM orders WHERE order_date::date >= $1 AND order_date::date <= $2",
            [date1, date2]
        );
        res.send(result);
    } catch (err) {
...
router.get("/dateOrders", async (req, res) => {
    ...
    try {
        const result = await db.query(
            "SELECT * FROM orders WHERE order_date::date = $1",
            [date]
        );
        res.send(result);
    } catch (err) {
